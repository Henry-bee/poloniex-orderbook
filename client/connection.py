...
class Connection(Thread):
    def __init__(self, event_handler, url, log_level=logging.INFO, daemon=True, reconnect_interval=3):
        ...
        self.logger = Utils.getLogger(loggerName='POLONIEXDataRecorder',logLevel='INFO')
    ...
    def _on_message(self, ws, message):
        ...
        channel_name = CODE2SYMBOL[code]
        ticks = params[-1]

        for tick in ticks:
            if str(tick[0]) in ['i', 'o', 't']:
                self.event_handler(
                    code,
                    tick,
                    channel_name
    ...
    def send_event(self, event_name, channel_name):
    ...
    def send_ping(self):
        ...
        try:
            self.socket.send(json.dumps({'command': 'pusher:ping', 'data': ''}))
        except Exception as e:
